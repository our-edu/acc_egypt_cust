...
# Modified: Custom General Ledger — reads party/party_type from custom_party/custom_party_type
# in Journal Entry accounts (falls back to GL Entry party/party_type for old documents).
...
def get_gl_entries(filters, accounting_dimensions):
	...
	if accounting_dimensions:
		dimension_fields = ", ".join(f"gle.{d}" for d in accounting_dimensions) + ","
	...
	gl_entries = frappe.db.sql(
		f"""
		select
			gle.name as gl_entry, gle.posting_date, gle.account,
			COALESCE(NULLIF(jea.custom_party_type, ''), gle.party_type) as party_type,
			COALESCE(NULLIF(jea.custom_party, ''), gle.party) as party,
...
def get_conditions(filters):
	...
	if filters.get("categorize_by") == "Categorize by Party" and not filters.get("party_type"):
		conditions.append(
			"(gle.party_type in ('Customer', 'Supplier') OR jea.custom_party_type in ('Customer', 'Supplier'))"
		)

	if filters.get("party_type"):
		conditions.append(
			"(gle.party_type=%(party_type)s OR jea.custom_party_type=%(party_type)s)"
		)

	if filters.get("party"):
		conditions.append(
			"(gle.party in %(party)s OR jea.custom_party in %(party)s)"
		)
			...
			if not dimension.disabled and dimension.document_type != "Finance Book":
				if filters.get(dimension.fieldname):
					if frappe.get_cached_value("DocType", dimension.document_type, "is_tree"):
						...
						conditions.append(f"gle.{dimension.fieldname} in %({dimension.fieldname})s")
					else:
						conditions.append(f"gle.{dimension.fieldname} in %({dimension.fieldname})s")
