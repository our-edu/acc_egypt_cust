...
# Modified: Custom Bank Reconciliation Statement — from_date / to_date range filters
...
def execute(filters=None):
	...
	balance_as_per_system = get_balance_on(filters["account"], filters["to_date"])
...
def get_entries(filters):
	...
	entries = list(journal_entries) + list(payment_entries) + list(pos_entries) + list(purchase_invoices)
...
def get_journal_entries(filters):
	...
	return (
		...
		.where(
			...
			& (je.clearance_date.isnull() | (je.clearance_date > filters.to_date))
...
def get_payment_entries(filters):
	...
	return (
		...
		.where(
			...
			& (pe.clearance_date.isnull() | (pe.clearance_date > filters.to_date))
...
def get_purchase_invoices(filters):
	...
	return (
		...
		.where(
			...
			& (pi.clearance_date.isnull() | (pi.clearance_date > filters.to_date))
...
def get_pos_entries(filters):
	...
	return (
		...
		.where(
			...
			& (si_payment.clearance_date.isnull() | (si_payment.clearance_date > filters.to_date))
...
def get_amounts_not_reflected_in_system(filters):
	...
	je_amount = (
		...
		.where(
			...
			& (jea.account == filters.account)
			& (je.posting_date > filters.to_date)
			& (je.clearance_date <= filters.to_date)
			& (je.company == filters.company)
			& ((je.is_opening.isnull()) | (je.is_opening == "No"))
		)
		.run(as_dict=True)
	...
	pe_amount = (
		...
		.where(
			...
			& (pe.docstatus == 1)
			& (pe.posting_date > filters.to_date)
			& (pe.clearance_date <= filters.to_date)
			& (pe.company == filters.company)
		)
		.run(as_dict=True)
	)
	...
	pi_amount = (
		...
		.where(
			...
			& (pi.cash_bank_account == filters.account)
			& (pi.posting_date > filters.to_date)
			& (pi.clearance_date <= filters.to_date)
			& (pi.company == filters.company)
		)
	).run(as_dict=True)
	...
	amount = je_amount + pe_amount + pi_amount
	return amount
